...
# ╔════════════════════════════════════════════════════════════════╗
# ║  FOU TUNNEL SETUP (Optimized for Electron App Model)          ║
# ║  Iran & Kharej Setup Script (Foo-over-UDP Multi-Tunnel)       ║
# ╚════════════════════════════════════════════════════════════════╝
...
print_header() {
    ...
    echo -e "${CYAN}${BOLD}"
    echo " ╔════════════════════════════════════════════════╗"
    echo " ║       FOU TUNNEL SETUP (Electron Model)       ║"
    echo " ╚════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
install_prereqs() {
    ...
    if ! modprobe fou 2>/dev/null; then
        msg_warn "fou kernel module not available."
    else
        msg_ok "fou module loaded."
    fi
...
if [ "\${FOU_ENABLE}" = "yes" ]; then
    # Inbound FOU: match UDP dport + local real IP (Electron model)
    ...
    # Outbound FOU: match UDP dport + local real IP (Electron model)
    ...
    # Inbound RAW: match Protocol 47 + local real IP
    nft add rule ip \${NFT_TABLE} prerouting  iif "\${IF_WAN}" ip protocol gre ip saddr \${REMOTE_SPOOF} ip daddr \${LOCAL_REAL} ip saddr set \${REMOTE_REAL} notrack
    # Outbound RAW: match Protocol 47 + local real IP
    nft add rule ip \${NFT_TABLE} postrouting oif "\${IF_WAN}" ip protocol gre ip saddr \${LOCAL_REAL} ip daddr \${REMOTE_REAL} ip saddr set \${LOCAL_SPOOF} notrack
fi
...
create_keepalive_service() {
    ...
    local service_name="gre-keepalive-${tunnel_id}"
    local service_path="${SYSTEMD_DIR}/${service_name}.service"

    cat > "${service_path}" << EOF
[Unit]
Description=GRE Tunnel ${tunnel_id} Keep-Alive
After=gre-tunnel-${tunnel_id}.service
Requires=gre-tunnel-${tunnel_id}.service

[Service]
Type=simple
ExecStart=/bin/bash -c 'sleep \$(( ${tunnel_id} % 5 )); while true; do ping -c 1 -W 2 -I gre${tunnel_id} ${remote_tun_ip} >/dev/null 2>&1; sleep 3; done'
Restart=always
RestartSec=5
...
show_gre_review() {
    ...
    echo ""
    print_double_line
    echo -e " ${WHITE}${BOLD}  REVIEW YOUR FOU SETTINGS${NC}"
    print_double_line
    echo ""
    ...
    echo -e "    ${GREEN}✓${NC} MSS Clamping (dynamic: 1352)"
    echo -e "    ${GREEN}✓${NC} GRE Firewall (whitelist)"
    echo -e "    ${GREEN}✓${NC} Keepalive (10s × 3)"
    echo -e "    ${GREEN}✓${NC} Continuous Keep-Alive Service (ping every 3s)"
...
setup_iran() {
    print_header
    echo -e " ${GREEN}${BOLD}>>> Setup Iran Server (FOU)${NC}"
    echo ""
    print_line
    ...
    echo -e "\n ${MAGENTA}${BOLD}[5/5] FOU UDP Port${NC}"
    msg_info "Encapsulates GRE in UDP to evade DPI detection."
    read_input "FOU UDP Port" "51820" FOU_PORT
    FOU_ENABLE="yes"
    ...
    msg_ok "Keep-alive service created and started: gre-keepalive-${TUNNEL_ID}"
    ...
    echo ""
    print_double_line
    echo -e " ${GREEN}${BOLD}  Iran FOU Tunnel ${TUNNEL_ID} Setup Complete!${NC}"
    print_double_line
    echo ""
...
setup_kharej() {
    print_header
    echo -e " ${GREEN}${BOLD}>>> Setup Kharej Server (FOU)${NC}"
    echo ""
    print_line
    ...
    echo ""
    print_double_line
    echo -e " ${GREEN}${BOLD}  Kharej FOU Tunnel ${TUNNEL_ID} Setup Complete!${NC}"
    print_double_line
    echo ""
...
do_watchdog_status() {
    ...
    echo ""
    echo -e " ${DIM}Watchdog checks every 10s, restarts after 3 consecutive failures.${NC}"
    echo ""
}
...
do_dashboard() {
    ...
    if [ $total -gt 0 ]; then
        ...
        for svc in $(systemctl list-units --type=service --all --no-legend 2>/dev/null | grep "gre-tunnel-" | awk '{print $1}'); do
            ...
            # Keepalive
            local ka_st=$(systemctl is-active "gre-keepalive-${tid}.service" 2>/dev/null)
...
do_restart_all() {
    ...
    for svc in $(systemctl list-units --type=service --all --no-legend 2>/dev/null | grep "gre-tunnel-" | awk '{print $1}'); do
        ...
        if systemctl list-units --type=service --all --no-legend | grep -q "gre-keepalive-${tid}"; then
            systemctl restart "gre-keepalive-${tid}" 2>/dev/null
        fi
        msg_ok "${name} and its keep-alive restarted."
    done
}
...
do_stop_all() {
    ...
    for svc in $(systemctl list-units --type=service --all --no-legend 2>/dev/null | grep "gre-tunnel-" | awk '{print $1}'); do
        ...
        local tid="${name#gre-tunnel-}"
        if systemctl list-units --type=service --all --no-legend | grep -q "gre-keepalive-${tid}"; then
            systemctl stop "gre-keepalive-${tid}" 2>/dev/null
        fi
...
do_start_all() {
    ...
    for svc in $(systemctl list-units --type=service --all --no-legend 2>/dev/null | grep "gre-tunnel-" | awk '{print $1}'); do
        ...
        if systemctl list-units --type=service --all --no-legend | grep -q "gre-keepalive-${tid}"; then
            systemctl start "gre-keepalive-${tid}" 2>/dev/null
        fi
        msg_ok "${name} started."
    done
}
...
list_tunnels() {
    echo ""
    echo -e " ${CYAN}${BOLD}Active FOU Tunnels:${NC}"
    print_line
    ...
    [ $found -eq 0 ] && { msg_warn "No GRE/FOU tunnels found."; return 1; }
    echo ""
    return 0
}
...
do_restart() {
    ...
    msg_ok "${SELECTED_TUNNEL} and its keep-alive restarted."
}
...
do_stop() {
    ...
    local tid="${SELECTED_TUNNEL#gre-tunnel-}"
    if systemctl list-units --type=service --all --no-legend | grep -q "gre-keepalive-${tid}"; then
        systemctl stop "gre-keepalive-${tid}"
    fi
...
do_start() {
    ...
    local tid="${SELECTED_TUNNEL#gre-tunnel-}"
    if systemctl list-units --type=service --all --no-legend | grep -q "gre-keepalive-${tid}"; then
        systemctl start "gre-keepalive-${tid}"
        systemctl enable "gre-keepalive-${tid}" &>/dev/null
    fi
...
do_status() {
    ...
    if [ -f "$script" ]; then
        ...
        local fp=$(grep '^FOU_PORT=' "$script" | cut -d'"' -f2)
...
do_delete() {
    ...
    if [ "$confirm" = "DELETE" ]; then
        ...
        systemctl stop "gre-keepalive-${tid}" 2>/dev/null
        systemctl disable "gre-keepalive-${tid}" 2>/dev/null
        ...
        rm -f "${SYSTEMD_DIR}/gre-keepalive-${tid}.service"
        ...
        rm -f "/tmp/.wd_gre${tid}"
        systemctl daemon-reload
        msg_ok "${SELECTED_TUNNEL}, keep-alive, watchdog, and scripts deleted."
    else
        msg_warn "Cancelled."
    fi
}
...
main_menu() {
    while true; do
        ...
        echo ""
        echo -e " ${BOLD}${WHITE}Setup FOU${NC}"
